...
namespace HK
{
    public partial class FrmBuscarProductos : Form
    {
        ...
        List<Producto> Respaldo = new List<Producto>();
        ...
        private void txtCant_Validating(object sender, CancelEventArgs e)
        {
            ...
            //{
            //}
            producto = (Producto)this.bindingSource.Current;
            producto.Pedido = ((double?)Editor.Value).GetValueOrDefault();
        }
        ...
        private void Busqueda()
        {
            ...
            foreach (Producto p in Productos.Where(x => x.Pedido > 0))
            {
                Respaldo.Add(p);
            }
            ...
            foreach (Producto p in Respaldo)
            {
                Producto Item = Productos.FirstOrDefault(x => x.IdProducto == p.IdProducto);
                if (Item != null)
